IMPROVE_SEARCH
RRF fusion (as planned) — fixes the "espiritu santo" class of failure where keyword matches get buried.
A relevance-cliff signal — either a distance cutoff (drop results beyond ~0.87–0.90 cosine distance, tunable), or a visual break in the UI ("strong matches" vs "weaker matches"), or simply dimming low-scoring cards. Cheap, and directly targets your complaint.
Skip: embedding model swap, learned ranker — evidence says wrong bottleneck.
The caption-length/OOM question is a separate track (the indexer's still stopped, that decision is still pending).
